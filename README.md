...
- 任务结束后展示失败列表（视频名 + 失败原因）和跳过列表
- 支持一键“重跑失败项”：只把失败的视频重新排队处理，不重复处理成功项
- 结果预览：生成的 GIF/图片/MP4 直接在页面里预览，按输出文件夹分组（组间顺序与生成前的读取顺序一致，组内按文件名自然排序），每个文件夹可一键打开结果目录和源目录
- 支持多任务顺序队列和历史记录查看（历史任务也可查看结果与重跑）
...
| `keep_running` | 开启后，即使关闭浏览器页面任务也会继续运行到结束（不受心跳中断暂停/停止影响） |
...
`settings.json` 会在任务启动时保存当前配置，便于下次打开时自动回填。`task_history.json` 会保存最近的任务历史、日志摘要和每个视频的处理结果（含输出文件列表，用于结果预览与失败重跑），随任务量增大文件会变大。这两个文件都已在 `.gitignore` 中忽略，不会进入版本控制。
...
- 如果想关闭浏览器页面后任务继续跑完，勾选“关闭页面后继续”：任务不再因心跳中断暂停或停止，程序会在任务运行期间自动保活，任务结束后 5 分钟无活动才自动退出。
